function initTable(instituteName) {
	//先销毁表格  
	$('#teacherTable').bootstrapTable('destroy');
	//初始化表格,动态从服务器加载数据  
	$("#teacherTable").bootstrapTable({
		method: "get", //使用get请求到服务器获取数据  
		url: "teacher.do?dispatch=toTeacherTable", //获取数据的Servlet地址  
...
function operateFormatter(value, row, index) {
    return [
        '<button type="button" class="viewTeacherDetail btn btn-default  btn-sm" style="margin-left:15px;">查 看</button>',
        '<button type="button" class="deleteTeacher btn btn-default  btn-sm" style="margin-left:15px;">删 除</button>',
   ].join('');
};

//为查看和删除按钮添加事件
window.operateEvents = {
	'click .viewTeacherDetail': function (e, value, row, index) {
